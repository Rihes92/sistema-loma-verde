...
//  · El modelo se descubre AUTOMÁTICAMENTE: se lista qué modelos flash
//    (capa gratuita) admite la clave del docente y se elige el mejor,
//    porque Google cambia y retira nombres de modelos con frecuencia.
//    Se puede forzar uno con la env GEMINI_MODEL.
...
// Modelo de Gemini. Los nombres cambian seguido y algunos dejan de estar
// disponibles para claves nuevas, por eso NO fijamos uno: si GEMINI_MODEL está
// definido se respeta; si no, se descubre un modelo "flash" (gratis) disponible
// para la clave de CADA docente. Se cachea por clave dentro de la instancia.
const _modeloCache = {};
async function elegirModelo(apiKey) {
  if (process.env.GEMINI_MODEL) return process.env.GEMINI_MODEL;
  if (_modeloCache[apiKey]) return _modeloCache[apiKey];
  try {
    const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models?pageSize=200&key=' + encodeURIComponent(apiKey));
    const j = await r.json();
    const cands = ((j && j.models) || [])
      .filter(m => Array.isArray(m.supportedGenerationMethods) && m.supportedGenerationMethods.includes('generateContent'))
      .map(m => String(m.name || '').replace(/^models\//, ''))
      .filter(n => /flash/i.test(n) && !/pro/i.test(n) && !/(embedding|aqa|imagen|image|tts|audio|vision|learnlm|gemma)/i.test(n));
    const score = (n) => {
      let s = 0;
      if (/flash-latest/i.test(n)) s += 1000;          // alias rodante: ideal
      const v = (n.match(/(\d+(?:\.\d+)?)/) || [])[1];
      if (v) s += parseFloat(v) * 10;                  // versión más nueva
      if (!/(preview|exp)/i.test(n)) s += 5;           // preferir estables
      if (/lite/i.test(n)) s += 1;                     // desempate: lite (más cupo)
      return s;
    };
    cands.sort((a, b) => score(b) - score(a));
    if (cands.length) { _modeloCache[apiKey] = cands[0]; return cands[0]; }
  } catch (_) {}
  return 'gemini-2.5-flash-lite'; // respaldo si el listado falla
}
...
module.exports = async (req, res) => {
  ...
  const payload = {
    ...
    // maxOutputTokens amplio: un planeador denso ocupa varios miles de tokens
    // (incluye el "pensamiento" de los modelos que lo usan). Sin thinkingConfig,
    // para ser compatible con cualquier modelo flash que elija la cuenta.
    generationConfig: {
      temperature: 0.7,
